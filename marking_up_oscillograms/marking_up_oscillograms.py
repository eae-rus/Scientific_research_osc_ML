...
class ComtradeProcessor(RawToCSV):
    ...
    def normalize_signals(self, df_osc: pd.DataFrame, name_osc: str):
        ...
        if norm_osc_row["norm"].values[0] != "ДА":
            return df_osc  # Пропуск не подготовленных наборов
...
class ComtradePredictionAndPlotting(ComtradeProcessor):
    ...
    def __init__(self, model_path: str, osc_name: str, uses_bus: str, strat_point: int, end_point: int, 
                 norm_file_path: str, device: str = 'cuda', f_networks = 50, ADC_sampling_rate = 1600):
        super().__init__(norm_file_path, device)
    ...
    def predict(self):
        ...
        # Initializing an empty DataFrame with the required columns, if present
        ...
        # Initialization of predicted labels
        predict_labels = {name: np.zeros(len(df_osc)) for name in FeaturesForDataset.FEATURES_TARGET}
