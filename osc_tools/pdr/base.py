...
class PDRDirection(IntEnum):
    ...
    - UNLABELED (-999): Неразмеченная область (прогрев 10 периодов, недостаточная предыстория).
      Специальный код для исключения путаницы с нулем (BLOCK).
    - REVERSE (-1): Обратное направление (КЗ за спиной / блокировка срабатывания).
    - BLOCK (0): Блокировка органа (сигналы ниже порога чувствительности, неопределенный угол).
    - FORWARD (1): Прямое направление (КЗ в зоне действия / срабатывание).
    """
...
class PDROutput:
    ...
    # Решение по направлению (FORWARD, REVERSE, BLOCK, UNLABELED)
    direction: PDRDirection
    # Факт пуска/срабатывания (True если FORWARD)
    is_tripped: bool
