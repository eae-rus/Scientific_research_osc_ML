...
# Сигналы делятся на эти значения перед FFT, чтобы привести масштаб
# к тому же диапазону, что и реальные осциллограммы после NormOsc.
#
# Логика: воспроизводим формулы NormOsc (normalize_bus_signals):
#   - Напряжения: divisor = 3 × Ub_base   → для SimOZZ: 3 × U_лин
#   - Фазные токи: divisor = 20 × Ip_base → для SimOZZ: 20 × I_ном_ТТ
#   - Ток 3I0:     divisor = 5 × Iz_base  → для SimOZZ: 5 × I_ном_ТТНП
#
# Подтверждено скриптом diagnose_scale_mismatch.py (2026-04-27).
# Константы хранятся ТОЛЬКО здесь; все потребители импортируют SIM_NOMINAL.
# ---------------------------------------------------------------------------

# Базовые номиналы оборудования SimOZZ (сеть 10 кВ)
_U_LIN_KV = 10.0         # Линейное напряжение, кВ
_I_NOM_CT_KA = 0.2       # Номинал ТТ = 200 А → 0.2 кА
_I_NOM_CTUP_KA = 0.03    # Номинал ТТНП = 30 А → 0.03 кА

# Множители (аналогичны NormOsc):
_K_VOLTAGE = 3            # Напряжения: divisor = 3 × base
_K_CURRENT = 20           # Фазные токи: divisor = 20 × base
_K_CURRENT_I0 = 5         # Ток 3I0: divisor = 5 × base

SIM_NOMINAL: Dict[str, float] = {
    # Фазные напряжения: 3 × U_лин = 30 кВ
    'UA': _K_VOLTAGE * _U_LIN_KV,
    'UB': _K_VOLTAGE * _U_LIN_KV,
    'UC': _K_VOLTAGE * _U_LIN_KV,
    # UN (3U0): 3 × утроенное фазное = 3 × (3 × U_лин / √3) ≈ 51.96 кВ
    'UN': _K_VOLTAGE * 3.0 * _U_LIN_KV / 1.7320508075688772,
    # Фазные токи: 20 × 0.2 = 4.0 кА
    'IA': _K_CURRENT * _I_NOM_CT_KA,
    'IB': _K_CURRENT * _I_NOM_CT_KA,
    'IC': _K_CURRENT * _I_NOM_CT_KA,
    # Ток нулевой последовательности: 5 × 0.03 = 0.15 кА
    'IN': _K_CURRENT_I0 * _I_NOM_CTUP_KA,
}
