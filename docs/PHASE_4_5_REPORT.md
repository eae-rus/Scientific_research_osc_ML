...
Технически это должно переиспользовать уже существующие функции
`load_comtrade_raw()` и `mark_real_oscillogram()` из `inference_real_ozz.py`, а не
дублировать pipeline обработки COMTRADE.
...
1. Реализовать `export_single_comtrade_marking.py` для обработки одного COMTRADE и
  сохранения CSV + PNG (некоторая часть уже есть, возможно надо будет перенести эти знания).
2. Добавить единый `model_complexity_report.py`, который для любого чекпоинта
  считает параметры, размер, latency, throughput и peak VRAM.
3. Подготовить конфиги для трёх baseline-моделей из §18.5 без дублирования
  тренировочного pipeline.
4. Добавить режим hard-negative fine-tuning на реальных `no OZZ` осциллограммах.
5. Сформировать таблицу результатов для статьи:
  - SimOZZ F1/ROC-AUC;
