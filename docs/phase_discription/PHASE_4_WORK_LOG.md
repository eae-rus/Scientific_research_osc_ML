...
# 2026-07-03 — перезапуск опыта насыщения ТТ (v2)

- Предыдущие CT-опыты признаны невалидными; старые checkpoint не подхватываются.
- Зафиксирован критерий разметки научного руководителя: 10 % мгновенной погрешности.
- Окно сокращено до 40 мс, шаг увеличен до 1/32 периода; FFT считается по предшествующему
  окну, поэтому все 64 токена валидны и не используют будущее.
- На вход возвращены V2; нормирование: I/100, U/300. I1 остаётся только эталоном для графика разности.
- Добавлены spectral Physical KAN (168×64) и raw Baseline (6×64); spectral-конфигурация согласована с
  SimOZZ: d_model=48, 4 головы, 6 слоёв.
- Аугментации: синхронная циклическая перестановка фаз и общий gain jitter 0,9–1,1; отключения фаз нет.
- В анализ добавлены график разности I1/1000−I2 и строгая нормировка реальных CSV по паспортным коэффициентам.

# 2026-07-05 — итоговые материалы раздела 7

- Полный holdout из 22 908 симулированных осциллограмм: позонно F1=0,681;
  на уровне файла Precision=0,993, Recall=0,814, F1=0,895.
- Подготовлены рисунки 11–14 и ручной сценарий
  `scripts/visualization/replot_article4_ct_saturation.py`.
- Воспроизводимые имена примеров, checkpoint, масштабы и zoom зафиксированы в
  `reports/phase4/article4_ct_saturation/FIGURE_MANIFEST.json` и `ARTICLE_FIGURES.md`.
- Аудит двух выбранных реальных записей подтвердил делители I/100 и U/300;
  повторной нормировки нет.
- Раздел 7 переписан в `(ч4) Трансформеры для детекции событий v0.4.docx`.
