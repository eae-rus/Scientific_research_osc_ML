...
- Проведён детальный анализ прогона 200 эпох лёгкой модели (`pretrain_b_small`, `d_model=64`, ~300k параметров, `cosine_restarts` каждые 50 эпох):
  ...
  - **Рассогласование источников после 90-й эпохи**:
    - Ошибка на **French/RTE** продолжала падать до 199 эпохи (`0.0003853`).
    - Ошибка на **Open_EE** выросла с `0.0003875` (эпоха 90) до `0.001291` (эпоха 199).
  - **Причины расхождения**:
    1. *Амплитудный дисбаланс и аномалии*: Медиана токов в Open_EE равна всего `0.0062` p.u. (тихий доаварийный режим), но встречаются единичные аномальные выбросы до `142.3` p.u. во французском же датасете медиана составляет `0.081` p.u., а максимум при КЗ `21.8` p.u.
    2. *Сэмплирование окон*: Подтверждена детерминированная случайная выборка окон по всей длине осциллограммы (`start = rng.randrange(...)`), проблемы со сдвигом только в начало нет.
    3. *Ограниченность емкости лёгкой модели*: `d_model=64` исчерпывает емкость к ~90 эпохе (1.8 млн окон) на 48 тыс. разнородных процессов.
- **Вывод для следующего этапа**: Для средних (`medium`) и тяжёлых (`heavy`) моделей с увеличенной емкостью сходимость будет более устойчивой. Рекомендована санитаризация аномальных выбросов Open_EE (>10-20 p.u.) при формировании входных окон.
