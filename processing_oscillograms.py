...
class ProcessingOscillograms():
    """
    Класс для обработки осциллограмм формата COMTRADE для дальнейшего публичного использования.
    """
    def __init__(self):
        pass
        # пока ничего нет. В будущем надо будет переработать подход передаваемых переменных.
                    ...
                    if file.endswith(".cfg"):  # Если файл имеет расширение .cfg
                        ...
                        try: 
                            ...
                            try:
                                ...
                                try:
                                    self._deleting_confidential_information_in_one_file(file_path, root, 'ОЕМ 866') # ОЕМ - русский язык
                                except Exception as e:
    ...
    def rename_analog_signals(self, source_dir: str, csv_dir: str, encoding: str = 'utf8', delimiter: str = ',') -> None:
        ...
        Args:
            ...
            encoding (str, optional): кодировка csv файла. Defaults to 'utf8'.
            delimiter (str, optional): разделитель в csv файла. Defaults to ','.
