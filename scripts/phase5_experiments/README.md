...
Текущий пакет содержит 100 файлов: 74 ранее размеченных и 26 новых, без
повторов внутри выборки. Рабочие метки помещаются в
`pdr_manual_blind_audit_v1/labels/<status>/blind_control/<source>`, после чего
запускается `import_pdr_manual_labels.py --blind-audit`. Код служебно разделит
повторные и новые случаи только после завершения разметки.
...
- `PHASE_5_PDR_ALGORITHMS.md` — математический контракт пяти РНМ;
