...
Запускаются тесты через ноутбук pytest_runner.ipynb, пробуй делать через него по очереди выполняя ячейки до "запуск всех тестов", она составляет отчёт в папку repory - после чего можно глянуть этот отчёт и уже по нему принимать какие-либо решения.

- **Не делаем**: Не запускай тесты сама напрямую из командной строки чтобы избежать проблем с окружением.
```
