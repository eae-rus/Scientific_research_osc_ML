                ...
                for root, dirs, files in os.walk(source_dir):  # Итерируемся по всем файлам и директориям в исходной директории
                    for file in files:  # Имя каждого файла
                        ...
                        if use_comtrade and file_lower.endswith(self.CFG_EXTENSION):  # Если файл имеет расширение .cfg
                            count_new_files += _process_file(self, file=file, root=root, source_dir=source_dir, dest_dir=dest_dir, copied_hashes=copied_hashes, 
                            ...
                            count_new_files += _process_archive_file(self, file=file, root=root, source_dir=source_dir, dest_dir=dest_dir, copied_hashes=copied_hashes, 
                                                                     preserve_dir_structure=preserve_dir_structure, use_hashes=use_hashes, _new_copied_hashes=_new_copied_hashes,
            ...
            for root, dirs, files in os.walk(source_dir):  # Итерируемся по всем файлам и директориям в исходной директории
                for file in files:  # Имя каждого файла
                    ...
                    if use_comtrade and file_lower.endswith(self.CFG_EXTENSION):  # Если файл имеет расширение .cfg
                        ...
                        count_new_files += _process_file(file=file, root=root, source_dir=source_dir, dest_dir=dest_dir, copied_hashes=copied_hashes, 
                                                         preserve_dir_structure=preserve_dir_structure, use_hashes=use_hashes, _new_copied_hashes=_new_copied_hashes,
                                                         type_osc=TYPE_OSC.EKRA)
...
def _process_file(self, file: str, root: str, source_dir: str, dest_dir: str, copied_hashes: dict = {}, 
    ...
    match type_osc:
        case TYPE_OSC.COMTRADE_CFG_DAT:
            return _process_comtrade_file(self, file=file, root=root, source_dir=source_dir, dest_dir=dest_dir, copied_hashes=copied_hashes, 
                                         preserve_dir_structure=preserve_dir_structure, use_hashes=use_hashes, _new_copied_hashes=_new_copied_hashes)
...
def match_oscillograms_to_terminals(self, dest_dir: str, copied_hashes: dict, terminal_oscillogram_names: dict) -> None:
    """
...
def organize_oscillograms_by_terminal(self, source_dir: str, dest_dir: str, terminal_list: list, terminal_oscillogram_names: dict) -> None:
    """
    ...
    with tqdm(total=total_files, desc="Organize oscillograms") as pbar:  # Инициализация прогресс-бара
        for root, dirs, files in os.walk(source_dir):  # Итерируемся по всем файлам и директориям в исходной директории
            for file in files:  # Имя каждого файла
                pbar.update(1)  # Обновление прогресс-бара на один файл
                if file.endswith(self.CFG_EXTENSION) and file[:-4] in osc_terminal_dict:  # Если файл имеет расширение .cfg и есть в списке имён интересующих осциллограмм
                    file_name = file[:-4]
                    cfg_file_name = file
