...
> **Контекст:** Проект находится в фазе активной разработки ML-моделей. Тестирование должно фокусироваться на стабильных компонентах, а не на экспериментальном ML-коде.
...
2. [Текущее состояние](#-текущее-состояние-58-тестов)
...
## 📊 Текущее состояние (~210 тестов)

### Структура тестов
```
tests/
├── conftest.py                          # ✅ Глобальные fixtures + 7 новых
...
│   ├── test_features_pdr_complete.py    # 🆕 ~30 тестов (Layer 1/2)
│   ├── test_preprocessing_filtering.py  # ✅ 11 тестов
│   ├── test_preprocessing_segmentation.py # ✅ ~30 тестов
│   ├── test_comtrade_parser.py          # ✅ 11 тестов
│   ├── test_io_comtrade_parser_complete.py # 🆕 ~15 тестов (Layer 1)
│   ├── test_analysis_overvoltage.py     # ✅ ~16 тестов
│   ├── test_analysis_motor_and_spef.py  # ✅ ~15 тестов
│   ├── test_features_normalization.py   # 🆕 ~15 тестов (Layer 2, с фикстурами)
│   ├── test_data_management_renaming.py # ✅ 16 тестов
│   ├── test_data_management_complete.py # 🆕 ~15 тестов (Layer 1)
│   └── test_ml_models_basic.py          # 🆕 ~38 тестов (Layer 3 smoke)
└── integration/
...
| Метрика | Значение |
|---------|----------|
| Всего тестов | **~210** (было 58, добавлено ~150+) |
| Pass rate | **~90%** (22 упавших из-за неполных фикстур и новых тестов) |
| Покрытие (approx) | ~45% |
| Время выполнения | ~30 сек |

**Примечание:** 22 падающих теста — это новые тесты, которые нужно исправить с использованием добавленных фикстур.
...
│   └── comtrade_custom.py             📅 Нужны тесты
│
├── io/                             ← ПРИОРИТЕТ 1
...
│   └── segmentation.py                🔜 NEXT (~10 тестов)
│
├── analysis/                       ← ПРИОРИТЕТ 1
│   ├── overvoltage.py                 📅 ~8 тестов
│   ├── detect_motor_starts.py         📅 ~5 тестов
│   └── spef_finder.py                 📅 ~5 тестов
│
├── features/                       ← ПРИОРИТЕТ 2 (адаптивный слой)
│   ├── pdr_calculator.py              ✅ 17 тестов (DONE)
│   └── normalization.py               📅 ~10 тестов (может измениться)
│
